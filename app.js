...
async function createCohort(){
  ...
  try{
    const{error}=await sb.from('cohorts').insert({name:n,organisation:o,facilitator_id:S.user.id});
    if(error)throw error;
    await renderDash();
  }catch(e){
    b.textContent='Create →';b.disabled=false;
    alert('Could not create cohort: '+(e.message||'Unknown error'));
  }
}
