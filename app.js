    ...
    // Usage: <NFLogo size={28} /> — drops in anywhere ◈ was used
    function NFLogo({ size = 28, animated = false }) {
      const s = size;
      // Geometry: outer hex r = s*0.44, mid r = s*0.255, center at s/2,s/2
      const cx = s / 2, cy = s / 2;
      const ro = s * 0.44, rm = s * 0.255;
      const hex = (r, phase = 0) =>
        [0,1,2,3,4,5].map(i => {
      ...
      return (
        <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} fill="none"
          ...
          <defs>
            <radialGradient id={`${uid}bg`} cx="50%" cy="50%" r="65%">
              <stop offset="0%" stopColor="#071020"/>
              <stop offset="100%" stopColor="#03070F"/>
            </radialGradient>
            <radialGradient id={`${uid}core`} cx="38%" cy="36%" r="60%">
              <stop offset="0%" stopColor="#F0D880"/>
              <stop offset="55%" stopColor="#C4A050"/>
              <stop offset="100%" stopColor="#8A6A28"/>
            </radialGradient>
            <linearGradient id={`${uid}spoke`} x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#E2BE78" stopOpacity="0.9"/>
              <stop offset="100%" stopColor="#C4A050" stopOpacity="0.45"/>
            </linearGradient>
          </defs>
          {/* Tile background */}
          <rect width={s} height={s} rx={s * 0.19} fill={`url(#${uid}bg)`}/>
          {/* Skip connections */}
          {[[0,3],[1,4],[2,5]].map(([a,b],i) => (
            <line key={i} x1={outer[a][0]} y1={outer[a][1]} x2={outer[b][0]} y2={outer[b][1]}
              stroke="#E2BE78" strokeWidth={s*0.006} opacity="0.13"/>
          ))}
          {/* Hex perimeter */}
          {outer.map((pt,i) => {
            const next = outer[(i+1)%6];
            return <line key={i} x1={pt[0]} y1={pt[1]} x2={next[0]} y2={next[1]}
              stroke="#C4A050" strokeWidth={s*0.007} opacity="0.3"/>;
          })}
          {/* Spokes center→outer */}
          {outer.map((pt,i) => (
            <line key={i} x1={cx} y1={cy} x2={pt[0]} y2={pt[1]}
              stroke={`url(#${uid}spoke)`} strokeWidth={s*0.009} opacity="0.65"/>
          ))}
          {/* Mid-ring dashed spokes */}
          {mid.map((pt,i) => (
            <line key={i} x1={cx} y1={cy} x2={pt[0]} y2={pt[1]}
              stroke="#C4A050" strokeWidth={s*0.007} strokeDasharray={`${s*0.014} ${s*0.028}`} opacity="0.32"/>
          ))}
          {/* Outer node halos */}
          {outer.map((pt,i) => (
            <circle key={i} cx={pt[0]} cy={pt[1]} r={s*0.065} fill="rgba(196,160,80,0.1)"/>
          ))}
          {/* Outer nodes */}
          {outer.map((pt,i) => (
            <circle key={i} cx={pt[0]} cy={pt[1]} r={s*0.038} fill="#C4A050"/>
          ))}
          {/* Outer node highlights */}
          {outer.map((pt,i) => (
            <circle key={i} cx={pt[0]} cy={pt[1]} r={s*0.016} fill="#F0D880"/>
          ))}
          {/* Mid-ring nodes */}
          {mid.map((pt,i) => (
            <circle key={i} cx={pt[0]} cy={pt[1]} r={s*0.024} fill="rgba(196,160,80,0.52)"/>
          ))}
          {/* Core halos */}
          <circle cx={cx} cy={cy} r={s*0.19} fill="rgba(196,160,80,0.06)"/>
          <circle cx={cx} cy={cy} r={s*0.12} fill="rgba(196,160,80,0.09)"/>
          {/* Core ring */}
          <circle cx={cx} cy={cy} r={s*0.082} fill="#03070F" stroke="#C4A050" strokeWidth={s*0.012}/>
          {/* Core fill */}
          <circle cx={cx} cy={cy} r={s*0.054} fill={`url(#${uid}core)`}/>
          {/* Specular highlight */}
          <circle cx={cx - s*0.016} cy={cy - s*0.016} r={s*0.022} fill="rgba(255,245,210,0.6)"/>
        </svg>
      );
    }
