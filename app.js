...
//  CFI ASSESSMENT
// ============================================================
function CFIView({ setView, items, cfiAnswers, setCfiAnswers, cfiStep, setCfiStep, cfiResult, setCfiResult, calculateCFI, isPro, user }) {
      ...
      {showIntro ? (
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:12 }}>
            ...
            <button onClick={async ()=>{ if(cfiStep<items.length-1){ setCfiStep(s=>s+1); } else { const r=calculateCFI(); setCfiResult(r); if(r&&user){ try{ await saveCFIResult(user.id,r,cfiAnswers); }catch(e){} } }}}
  ...
  return (
    <div style={{ background:T.navy, minHeight:"100vh", color:T.text, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"0 16px 120px" }}>
        ...
        {view==="cfi"         && <CFIView setView={setView} items={CFI_ITEMS} cfiAnswers={cfiAnswers} setCfiAnswers={setCfiAnswers} cfiStep={cfiStep} setCfiStep={setCfiStep} cfiResult={cfiResult} setCfiResult={setCfiResult} calculateCFI={calculateCFI} isPro={isPro} user={user} />}
