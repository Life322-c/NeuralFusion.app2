...
function ResetPasswordModal({ onClose }) {
  ...
  const handleReset = async () => {
    if (!password || !confirm) { setMsg({ text: "Both fields required.", type: "error" }); return; }
    ...
    else { setMsg({ text: "Password updated! You can now sign in.", type: "success" }); setTimeout(onClose, 2000); }
  };
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(7,15,30,0.96)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ ...card(), maxWidth:420, width:"100%", padding:"36px 28px", animation:"fadeIn 0.35s ease" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
          <div>
            <div style={{ ...serif, fontSize:22, color:T.text, fontWeight:700 }}>Set New Password</div>
            <div style={{ color:T.muted, fontSize:12, ...sans }}>Choose a new password for your account</div>
          </div>
          <button onClick={onClose} style={{ background:"none", border:"none", color:T.muted, fontSize:22, cursor:"pointer" }}>×</button>
        </div>
        <div style={{ marginBottom:14 }}>
          <label style={{ color:T.muted2, fontSize:12, fontWeight:600, display:"block", marginBottom:6, ...sans }}>NEW PASSWORD</label>
          <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" style={{ ...inp() }} />
        </div>
        <div style={{ marginBottom:20 }}>
          <label style={{ color:T.muted2, fontSize:12, fontWeight:600, display:"block", marginBottom:6, ...sans }}>CONFIRM PASSWORD</label>
        ...
        <button onClick={handleReset} style={{ ...goldBtn(), width:"100%", padding:"14px", fontSize:15, opacity: loading?0.6:1 }} disabled={loading}>
...
function AuthModal({ onClose, onAuthSuccess }) {
  ...
  const handleForgotPassword = async () => {
    ...
    setLoading(false);
    if (error) setMsg({ text: error.message, type: "error" });
    else setMsg({ text: "Reset link sent! Check your email.", type: "success" });
  };
      ...
      <div style={{ ...card(), maxWidth:420, width:"100%", padding:"36px 28px", animation:"fadeIn 0.35s ease" }}>
        ...
        {tab === "login" && (
          <div style={{ textAlign:"right", marginBottom:20 }}>
            <span onClick={handleForgotPassword} style={{ color:T.gold, fontSize:12, cursor:"pointer", ...sans }}>Forgot password?</span>
          </div>
        )}
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = sb.auth.onAuthStateChange((event, session) => {
      ...
      if (u) {
        ...
        if (event === "PASSWORD_RECOVERY") { setShowAuth(false); setShowResetPassword(true); }
