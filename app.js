const { useState, useEffect, useCallback, useRef, useMemo } = React;
    ...
    function AuthModal({ onClose, onSuccess, initialTab = 'login' }) {
      ...
      const handleSignup = async () => {
        ...
        try {
          ...
          if (needsConfirm) {
            setLoading(false);
            setMsg({text:'Account created! Check your inbox and click the confirmation link before signing in.', type:'success'});
            return;
          }
      ...
      const handleForgotPassword = async () => {
        ...
        setLoading(false);
        if (error) setMsg({text:error.message,type:'error'});
        else setMsg({text:'Reset link sent! Check your inbox and follow the link to set a new password.', type:'success'});
      };

      const handleUpdatePassword = async () => {
        if (!password) { setMsg({text:'New password is required.',type:'error'}); return; }
        if (password.length < 8) { setMsg({text:'Password must be at least 8 characters.',type:'error'}); return; }
        ...
        if (error) setMsg({text:error.message,type:'error'});
          ...
          setMsg({text:'Password updated successfully! You are now signed in.', type:'success'});
          setTimeout(()=>{ onSuccess(); onClose(); }, 1500);
        }
      };

      const tabMeta = {
        login:           { title:'Access Your System',      sub:'Enter credentials to resume cognitive training.' },
        signup:          { title:'Initialize Profile',       sub:'Create your cognitive profile to begin.' },
        'forgot-password':{ title:'Reset Password',          sub:'Enter your email to receive a reset link.' },
        'update-password':{ title:'Set New Password',        sub:'Choose a strong new password for your account.' },
      };
    ...
    function Navbar({ view, setView, user, profile, setShowAuth, onSignOut }) {
      ...
      const navItems = [
        ...
        { v:'cfi', label:'Pressure Mode' },
        { v:'analytics', label:'Evolution' },
    ...
    function BottomNav({ view, setView }) {
      const items = [
        ...
        { v:'four-brains', symbol:'◈', label:'Architect' },
        { v:'cfi', symbol:'◎', label:'Pressure' },
...
function HomeView({ setView, user, setShowAuth, cfiResult, lessonProgress, sessions }) {
  ...
  const steps = [
    ...
    { step:'04', title:'Lesson Manuals',      desc:'Elite training documents from foundation to mastery.', view:'lessons' },
  ...
  /* ── Protocols list ───────────── */
  const protocols = [
    { icon:'◰', label:'ANALYTICAL', title:'Structure Protocol', desc:'Decompose any situation into verifiable facts. Eliminate interpretation. Build precision from raw data.', points:['Logical deduction drills','Pattern verification','Systematic planning'], color:'#C4A050' },
    { icon:'◱', label:'INTUITIVE',  title:'Signal Protocol',    desc:'Accelerate pattern recognition. Separate genuine intuitive signal from fear-driven noise.', points:['Rapid situational reads','Signal calibration','Accuracy logging'], color:'#E2BE78' },
    { icon:'◲', label:'ASSOCIATIVE',title:'Synthesis Protocol', desc:'Generate cross-domain connections invisible to linear thinkers. Transform divergence into precision.', points:['Creative constraint exercises','Idea compression','Divergence protocols'], color:'#7AAFCF' },
    { icon:'◳', label:'REFLECTIVE', title:'Awareness Protocol', desc:'Activate meta-cognitive observation. Extract lessons. Integrate experience into structured wisdom.', points:['Structured reflection','Rumination interruption','Clarity journaling'], color:'#D4AF6A' },
    { icon:'◈', label:'INTEGRATED', title:'FUSE Protocol',      desc:'Coordinate all four brains in sequence. Complete the Core Loop in under 90 seconds with training.', points:['Full four-brain activation','Commitment Lock','Fluency installation'], color:'#C4A050' },
    { icon:'◎', label:'PRESSURE',   title:'Stabilization',      desc:'Maintain integrated cognition under high-stakes conditions. Pressure is not the enemy; fragmentation is.', points:['Cognitive anchoring','Temporal compression','Mode regulation'], color:'#E2BE78' },
  ];

  /* ── Evolution timeline ───────── */
  const timeline = [
    { step:'01', title:'Diagnose',   desc:'Take the CFI™ Assessment. Identify your fragmentation band, dominant mode, and training priority.', view:'cfi' },
    { step:'02', title:'Understand', desc:'Study the Four Brains Framework. Learn the cognitive architecture you were never taught.', view:'four-brains' },
    { step:'03', title:'Train',      desc:'Run structured protocols. Activate modes deliberately. Build the Core Loop into muscle memory.', view:'training' },
    { step:'04', title:'Deepen',     desc:'Work through Lesson Manuals. Foundation to Mastery. Each document is a transformation, not information.', view:'lessons' },
    { step:'05', title:'Integrate',  desc:'FUSE becomes automatic. Cognitive fluency installs. Four brains, one unified intelligence system.', view:'analytics' },
  ];
...
a premium mental performance environment for{' '}
    ...
    function Footer({ setView }) {
      const links = [
        { label:'Dashboard', v:'home' },{ label:'Architecture', v:'four-brains' },
        { label:'Pressure Mode', v:'cfi' },{ label:'Protocols', v:'training' },
        { label:'Evolution', v:'analytics' },{ label:'Academy', v:'lessons' },
        { label:'About', v:'about' },
      ];
      const legalLinks = [
