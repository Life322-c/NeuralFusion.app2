...
// ── Supabase Config ──────────────────────────────────────────
const SUPABASE_URL  = "https://https://civwcmteqidppscbpqni.supabase.co";
...
function Navbar({ view, setView, isPro, onAdmin, user, setShowAuth, onSignOut }) {
  return (
    <nav style={{ background:"rgba(7,15,30,0.95)", borderBottom:`1px solid ${T.goldBorder}`, backdropFilter:"blur(12px)", position:"sticky", top:0, zIndex:100 }}>
      <div style={{ maxWidth:900, margin:"0 auto", padding:"0 16px", display:"flex", alignItems:"center", justifyContent:"space-between", height:60 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer" }} onClick={()=>setView("home")}>
          <div style={{ width:32, height:32, borderRadius:8, background:`linear-gradient(135deg,${T.gold},${T.goldLight})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 }}>⚡</div>
          <span style={{ ...serif, fontSize:18, color:T.text, fontWeight:700 }}>NeuralFusion™</span>
          {isPro && <span style={{ fontSize:10, background:T.gold, color:T.navy, padding:"2px 6px", borderRadius:4, fontWeight:700, ...sans }}>PRO</span>}
        </div>
        {/* Desktop nav */}
        <div style={{ display:"flex", gap:4, alignItems:"center" }}>
          {[["home","Home"],["lessons","Lessons"],["cfi","CFI™"],["history","Sessions"],["upgrade","Upgrade"]].map(([v,l]) => (
            <button key={v} onClick={()=>setView(v)} style={{ background:view===v?T.goldDim:"none", color:view===v?T.gold:T.muted2, border:"none", borderRadius:8, padding:"6px 14px", cursor:"pointer", fontSize:13, fontWeight:500, ...sans }}>{l}</button>
          ))}
          {user
            ? <button onClick={onSignOut} style={{ background:"none", border:"none", color:T.muted, cursor:"pointer", fontSize:12, ...sans, padding:"6px 10px" }}>Sign Out</button>
            : <button onClick={()=>setShowAuth(true)} style={{ ...outlineBtn(), padding:"6px 14px", fontSize:12 }}>Sign In</button>
          }
          <button onClick={onAdmin} style={{ background:"none", border:"none", color:T.muted, cursor:"pointer", fontSize:18, marginLeft:4 }}>⚙️</button>
        </div>
      </div>
    </nav>
  );
