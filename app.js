    ...
    // ── NEURALFUSION LOGO ICON (SVG) ─────────────────────────────────
    const _NF_OUTER = [0,1,2,3,4,5].map(i => {
      const a = (Math.PI / 3) * i;
      return [50 + 44 * Math.sin(a), 50 - 44 * Math.cos(a)];
    });
    const _NF_MID = [0,1,2,3,4,5].map(i => {
      const a = (Math.PI / 3) * i;
      return [50 + 25.5 * Math.sin(a), 50 - 25.5 * Math.cos(a)];
    });
    function NFLogoDefs() {
      return (
        <svg width="0" height="0" style={{position:'absolute',overflow:'hidden'}} aria-hidden="true">
          <defs>
            <radialGradient id="nfBg" cx="50%" cy="50%" r="65%">
              <stop offset="0%" stopColor="#071020"/>
              <stop offset="100%" stopColor="#03070F"/>
            </radialGradient>
            <radialGradient id="nfCore" cx="38%" cy="36%" r="60%">
              <stop offset="0%" stopColor="#F0D880"/>
              <stop offset="55%" stopColor="#C4A050"/>
              <stop offset="100%" stopColor="#8A6A28"/>
            </radialGradient>
            <linearGradient id="nfSpoke" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#E2BE78" stopOpacity="0.9"/>
              <stop offset="100%" stopColor="#C4A050" stopOpacity="0.45"/>
            </linearGradient>
            <symbol id="nfSymbol" viewBox="0 0 100 100">
              <rect width="100" height="100" rx="19" fill="url(#nfBg)"/>
              {[[0,3],[1,4],[2,5]].map(([a,b],i) => (
                <line key={i} x1={_NF_OUTER[a][0]} y1={_NF_OUTER[a][1]} x2={_NF_OUTER[b][0]} y2={_NF_OUTER[b][1]}
                  stroke="#E2BE78" strokeWidth="0.6" opacity="0.13"/>
              ))}
              {_NF_OUTER.map((pt,i) => {
                const nx = _NF_OUTER[(i+1)%6];
                return <line key={i} x1={pt[0]} y1={pt[1]} x2={nx[0]} y2={nx[1]}
                  stroke="#C4A050" strokeWidth="0.7" opacity="0.3"/>;
              })}
              {_NF_OUTER.map((pt,i) => (
                <line key={i} x1="50" y1="50" x2={pt[0]} y2={pt[1]}
                  stroke="url(#nfSpoke)" strokeWidth="0.9" opacity="0.65"/>
              ))}
              {_NF_MID.map((pt,i) => (
                <line key={i} x1="50" y1="50" x2={pt[0]} y2={pt[1]}
                  stroke="#C4A050" strokeWidth="0.7" strokeDasharray="1.4 2.8" opacity="0.32"/>
              ))}
              {_NF_OUTER.map((pt,i) => (
                <circle key={i} cx={pt[0]} cy={pt[1]} r="6.5" fill="rgba(196,160,80,0.1)"/>
              ))}
              {_NF_OUTER.map((pt,i) => (
                <circle key={i} cx={pt[0]} cy={pt[1]} r="3.8" fill="#C4A050"/>
              ))}
              {_NF_OUTER.map((pt,i) => (
                <circle key={i} cx={pt[0]} cy={pt[1]} r="1.6" fill="#F0D880"/>
              ))}
              {_NF_MID.map((pt,i) => (
                <circle key={i} cx={pt[0]} cy={pt[1]} r="2.4" fill="rgba(196,160,80,0.52)"/>
              ))}
              <circle cx="50" cy="50" r="19" fill="rgba(196,160,80,0.06)"/>
              <circle cx="50" cy="50" r="12" fill="rgba(196,160,80,0.09)"/>
              <circle cx="50" cy="50" r="8.2" fill="#03070F" stroke="#C4A050" strokeWidth="1.2"/>
              <circle cx="50" cy="50" r="5.4" fill="url(#nfCore)"/>
              <circle cx="48.4" cy="48.4" r="2.2" fill="rgba(255,245,210,0.6)"/>
            </symbol>
          </defs>
        </svg>
      );
    }
    const NFLogo = React.memo(function NFLogo({ size = 28, animated = false }) {
      return (
        <svg width={size} height={size} viewBox="0 0 100 100" fill="none"
          style={{ flexShrink:0, display:'block', animation: animated ? 'neuralPulse 3s ease-in-out infinite' : 'none' }}>
          <use href="#nfSymbol"/>
        </svg>
      );
    });
      ...
      return (
        <div style={{ width:size, height:size, position:'relative', display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          <div style={{
            ...
            <NFLogo size={Math.round(size*0.32)} animated={false} />
          </div>
          {/* Orbit dots */}
        ...
        <>
          <nav style={{
            ...
            <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 24px', height:60, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <button onClick={()=>setView('home')} style={{ background:'none', border:'none', display:'flex', alignItems:'center', gap:10, cursor:'pointer' }}>
                <NFLogo size={30} />
                <div>
        ...
        <div style={{ paddingTop:80, paddingBottom:100 }}>
          ...
          <section style={{ padding:'48px 24px 40px', borderBottom:`1px solid ${C.border}` }}>
            <div style={{ maxWidth:1200, margin:'0 auto' }}>
              ...
              <h1 style={{ ...syne, fontSize:32, fontWeight:800, color:C.text, lineHeight:1.05, marginBottom:16, maxWidth:700 }}>
          ...
          {user && cfiResult && (
            <section style={{ padding:'0 24px 80px', maxWidth:1200, margin:'0 auto' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap:16 }}>
                {[
                  { label:'CFI BAND', value:cfiResult.band, color:cfiResult.band==='Integrated'?'#7AAFCF':'#C4A050' },
                  { label:'LESSONS COMPLETED', value:`${completedLessons} / ${LESSONS.length}`, color:C.cyan },
                  { label:'SESSIONS LOGGED', value:sessions.length.toString(), color:'#E2BE78' },
          ...
          <section style={{ padding:'80px 24px', maxWidth:1200, margin:'0 auto' }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(300px, 100%), 1fr))', gap:48, alignItems:'center' }}>
              <div>
                ...
                <h2 style={{ ...syne, fontSize:26, fontWeight:800, color:C.text, lineHeight:1.1, marginBottom:24 }}>
          ...
          <section style={{ padding:'80px 24px', background:`linear-gradient(180deg, transparent, ${C.deep}80, transparent)` }}>
            <div style={{ maxWidth:1200, margin:'0 auto' }}>
              <div style={{ textAlign:'center', marginBottom:64 }}>
                ...
                <h2 style={{ ...syne, fontSize:26, fontWeight:800, color:C.text }}>How the system trains you</h2>
              </div>
          ...
          <section style={{ padding:'80px 24px', maxWidth:1200, margin:'0 auto' }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(300px, 100%), 1fr))', gap:48, alignItems:'center' }}>
              <div>
                ...
                <h2 style={{ ...syne, fontSize:24, fontWeight:800, color:C.text, lineHeight:1.15, marginBottom:16 }}>
              ...
              <div className="card" style={{ padding:'32px', position:'relative', overflow:'hidden' }}>
                ...
                {['Analytical Coherence', 'Intuitive Alignment', 'Associative Flexibility', 'Reflective Depth', 'Integration Stability'].map((dim,i)=>{
                  ...
                  const col = pct>=75?'#7AAFCF':pct>=60?C.cyan:'#C4A050';
                  return (
                ...
                <div style={{ marginTop:24, padding:'16px', background:C.deep, borderRadius:2, border:`1px solid ${C.border}` }}>
                  ...
                  <div style={{ ...syne, fontSize:18, fontWeight:800, color:'#C4A050' }}>Moderate Fragmentation</div>
                </div>
              </div>
            </div>
          </section>
          ...
          <section style={{ padding:'80px 24px', textAlign:'center', position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ position:'relative', zIndex:1, maxWidth:700, margin:'0 auto' }}>
              ...
              <h2 style={{ ...syne, fontSize:28, fontWeight:800, color:C.text, lineHeight:1.1, marginBottom:24 }}>
          ...
          <div style={{ maxWidth:1200, margin:'0 auto', padding:'40px 24px' }}>
            ...
            <div style={{ marginTop:64, padding:'48px 0', borderTop:`1px solid ${C.border}` }}>
              <div style={{ textAlign:'center', marginBottom:48 }}>
                ...
                <h2 style={{ ...syne, fontSize:24, fontWeight:800, color:C.text, lineHeight:1.15 }}>No brain is superior. All four must integrate.</h2>
              </div>
              ...
              <div className="card" style={{ padding:'40px', marginBottom:32, position:'relative', overflow:'hidden', borderColor:`${bandColor}22` }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap:32, alignItems:'center' }}>
                  <div>
                    ...
                    <div style={{ ...syne, fontSize:36, fontWeight:800, color:bandColor, lineHeight:1 }}>{cfiResult.total}</div>
          ...
          <div style={{ maxWidth:1200, margin:'0 auto', padding:'40px 24px' }}>
            ...
            {!cfiResult ? (
              <div className="card" style={{ padding:'60px', textAlign:'center' }}>
                <div style={{ ...mono, fontSize:30, color:C.dim, marginBottom:24 }}>◎</div>
                ...
                return (
                  <div key={lesson.id} className="card" style={{
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'auto 1fr auto', gap:24, alignItems:'center' }}>
                      <div style={{ ...syne, fontSize:30, fontWeight:800, color:col+'33', lineHeight:1 }}>
          ...
          <div style={{ maxWidth:1200, margin:'0 auto', padding:'60px 24px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(300px, 100%), 1fr))', gap:64, alignItems:'center', marginBottom:80 }}>
              ...
              <div className="card" style={{ padding:'40px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ ...syne, fontSize:20, fontWeight:800, color:C.text, lineHeight:1.3, marginBottom:24 }}>
      ...
      if (authLoading) return (
        <div style={{ background:C.void, minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center' }}>
            <NFLogo size={52} animated={true} />
            <div style={{ ...mono, fontSize:9, letterSpacing:3, color:C.muted, marginTop:20 }}>INITIALIZING SYSTEM...</div>
          </div>
        </div>
      );
