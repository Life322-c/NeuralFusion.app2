...
function paystack(){
  ...
  // Pre-load Paystack script eagerly so it's ready before user taps
  function doLaunch(){
    ...
    // Open immediately — no setTimeout — keeps it inside the user gesture
    handler.openIframe();
  }
  if(window.PaystackPop){
    doLaunch();
  }else{
    // Script not loaded yet — show loading state, load, then open
    ...
    s.onerror=function(){
      if(btn){btn.textContent='Unlock Now — ₦2,500';btn.disabled=false;}
      alert('Could not load payment. Check your internet and try again.');
    };
...
async function renderDash(){
  ...
  if(role==='admin'){
    ...
    const[{data:users},{data:payments},{data:cohorts},{data:cfis},{data:completions},{data:certs}]=await Promise.all([
    ...
    _dash={users:users||[],payments:payments||[],cohorts:cohorts||[],cfis:cfis||[],completions:completions||[],certs:certs||[],tab:_dash.tab||'overview'};
...
function _renderAdminShell(el){
  const tabs=[['overview','Overview'],['users','Users'],['cohorts','Cohorts'],['cfi','CFI Assessments'],['payments','Payments']];
  el.innerHTML=`
...
function dashTab(t){
  _dash.tab=t;
  // Update tab styles
  ['overview','users','cohorts','cfi','payments'].forEach(k=>{
...
function _renderAdminTab(t){
  ...
  if(!el)return;
  const{users,payments,cohorts,cfis,completions,certs}=_dash;
  ...
  if(t==='overview'){
    ...
    el.innerHTML=`
      ...
      ${!cohorts.length?'<div class="card"><p style="font-size:13px;color:var(--ink3);">No cohorts yet.</p></div>':cohorts.map(c=>{
        ...
        const enrolled=_dash.completions.filter(x=>x.cohort_id===c.id);
