...
// STATE
const S={user:null,profile:null,cohort:null,certs:[],done:new Set(),preCFI:null,postCFI:null,preA:null,postA:null,premium:false,lesson:null,cfiType:'pre',cfiA:[],cfiI:0,authMode:'login',role:'participant',_inOtpFlow:false};
...
function goAuth(mode){
  S.authMode=mode;
  document.getElementById('authHead').textContent='Sign In';
  ...
  document.getElementById('otpProfileStep').style.display='none';
  document.getElementById('aEmail').value='';
  document.getElementById('aOtp') && (document.getElementById('aOtp').value='');
  show('auth');
}
...
function resetOtpFlow(){
  document.getElementById('otpEmailStep').style.display='block';
  document.getElementById('otpVerifyStep').style.display='none';
  document.getElementById('aOtp').value='';
  document.getElementById('authMsg').style.display='none';
  document.getElementById('authHead').textContent='Sign In';
}
...
function showAuthMsg(msg,isErr){
  ...
  if(isErr){
    el.style.color='';el.style.background='';el.style.borderRadius='';el.style.padding='';
    el.classList.add('alert-err');
  }else{
    el.classList.remove('alert-err');
    el.style.color='#065F46';el.style.background='#D1FAE5';
    el.style.borderRadius='8px';el.style.padding='12px';
  }
}
...
async function sendOTP(){
  ...
  try{
    ...
    document.getElementById('aOtp').focus();
    showAuthMsg('Code sent! Check your inbox (and spam folder).',false);
...
async function verifyOTP(){
  ...
  try{
    ...
    if(!p||!p.full_name){
      // New user — show role selection + profile form
      S.role='participant';
      document.getElementById('rP').classList.add('active');
      document.getElementById('rF').classList.remove('active');
      document.getElementById('aName').value='';
      document.getElementById('aOrg').value='';
      document.getElementById('otpVerifyStep').style.display='none';
      document.getElementById('otpProfileStep').style.display='block';
      document.getElementById('authHead').textContent='About You';
      document.getElementById('authMsg').style.display='none';
    }else{
      // Returning user — show welcome message then load app
      showAuthMsg('✓ Welcome back, '+(p.full_name.split(' ')[0]||'')+'! Signing you in…',false);
      await _flushPendingCFI();
      setTimeout(()=>initUser(),900);
    }
  }catch(e){S._inOtpFlow=false;showAuthMsg(e.message,true);}
  finally{btn.textContent='Verify & Continue →';btn.disabled=false;}
}

async function saveProfile(){
  const name=document.getElementById('aName').value.trim();
  const org=document.getElementById('aOrg').value.trim();
  if(!name){showAuthMsg('Please enter your full name.',true);return;}
  const btn=document.getElementById('profileBtn');
  btn.textContent='Saving…';btn.disabled=true;
  try{
    await sb.from('profiles').upsert({id:S.user.id,full_name:name,role:S.role,organisation:org,is_premium:false});
    await _flushPendingCFI();
    showAuthMsg('✓ Welcome to NeuralFusion™, '+name.split(' ')[0]+'!',false);
    setTimeout(()=>initUser(),900);
  }catch(e){showAuthMsg(e.message,true);btn.textContent='Start Training →';btn.disabled=false;}
}

async function _flushPendingCFI(){
  if(!sb||!S.user||!S._pendingCFI)return;
  try{
    const{score,ans,type}=S._pendingCFI;
    const dims={dim_a:dSc(ans,'A'),dim_b:dSc(ans,'B'),dim_c:dSc(ans,'C'),dim_d:dSc(ans,'D'),dim_e:dSc(ans,'E')};
    await sb.from('cfi_assessments').insert({participant_id:S.user.id,cohort_id:S.cohort?.id||null,type,answers:ans,total_score:score,...dims});
    S._pendingCFI=null;
...
async function initUser(){
  if(_initRunning)return;_initRunning=true;
  try{
  ...
  const[{data:lc},{data:cfis},{data:certs},{data:en}]=await Promise.all([
    ...
    sb.from('enrollments').select('cohort_id,cohorts(*)').eq('participant_id',S.user.id).limit(1).maybeSingle()
  ]);
  ...
  S.certs=certs||[];
  if(en?.cohorts)S.cohort=en.cohorts;
  const handled=await checkPaymentReturn();
  if(!handled){if(!S.preCFI){startCFI('pre');}else{renderLessons();show('lessons');}}
...
// LESSONS
let _paystackLoaded=false;
function preloadPaystack(){
  if(_paystackLoaded||window.PaystackPop)return;
  _paystackLoaded=true;
...
async function saveSynth(){
  ...
  setTimeout(()=>{b.textContent='Save Session';b.style.background='';b.style.color='';b.disabled=false;},2000);
}
...
async function dashTab(t){
  ...
  await _renderAdminTab(t);
}

async function _renderAdminTab(t){
...
async function issueCert(pid,pre,post,cid){
  ...
  try{
    const{error}=await sb.from('certificates').insert({participant_id:pid,cohort_id:cid,issued_by:S.user.id,pre_cfi:pre,post_cfi:post,clarity_delta:pre-post});
    if(error)throw error;
    if(m){m.innerHTML='<div class="alert alert-ok">Certificate issued.</div>';setTimeout(()=>m.innerHTML='',3000);}
    const{data:cohort}=await sb.from('cohorts').select('*').eq('id',cid).single();
    if(cohort)openCohort(cohort);
  }catch(e){
    if(m){m.innerHTML=`<div class="alert alert-err">Failed to issue certificate: ${e.message||'Unknown error'}</div>`;setTimeout(()=>m.innerHTML='',4000);}
  }
}
...
async function joinCohort(){
  ...
  const{error:enErr}=await sb.from('enrollments').upsert({cohort_id:c.id,participant_id:S.user.id});
  if(enErr){err('enrollErr','Could not enroll: '+enErr.message);return;}
  S.cohort=c;
...
async function boot(){
  ...
  try{
    ...
    sb.auth.onAuthStateChange(async(_,session)=>{if(session&&!S.user&&!S._inOtpFlow)await initUser();else if(!session){S.user=null;S.profile=null;}});
  }catch(e){show('landing');}
}
