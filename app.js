...
function renderBnav(cur){
  ...
  const items=role==='facilitator'
    ...
    :[{i:'📚',l:'Learn',s:'lessons'},{i:'⚡',l:'Synthesis',s:'synthesis'},{i:'📈',l:'Progress',s:'progress'},{i:'🧠',l:'AI',s:'ai'},...(S.user&&!S.cohort?[{i:'🔗',l:'Cohort',s:'enroll'}]:[])];
  ...
  function launch(){
    const handler=PaystackPop.setup({
      ...
      callback:async function(r){
        ...
        S.premium=true;
        alert('Payment successful! Lessons 2-5 are now unlocked.');
...
async function renderDash(){
  const role=S.profile?.role;
  document.getElementById('dashRole').textContent=role==='admin'?'Admin Portal':'Facilitator';
  const el=document.getElementById('dashContent');
  el.innerHTML='<div class="dots" style="padding:40px 0;"><span></span><span></span><span></span></div>';
  if(role==='admin'){
    const[{data:users},{data:payments},{data:cohorts}]=await Promise.all([
    ...
    el.innerHTML=`
      <h2 style="font-family:'Fraunces',serif;font-size:22px;font-weight:600;margin-bottom:16px;">Admin Portal</h2>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:18px;">
        ${[['Users',(users||[]).length,'#6366F1'],['Premium',prem,'#10B981'],['Revenue',`₦${rev.toLocaleString()}`,'#C4952A']].map(([l,v,c])=>`<div class="card" style="text-align:center;padding:14px 8px;"><div style="font-family:'Fraunces',serif;font-size:26px;font-weight:700;color:${c};">${v}</div><div style="font-size:10px;color:var(--ink3);margin-top:2px;text-transform:uppercase;letter-spacing:.05em;">${l}</div></div>`).join('')}
      </div>
      <div class="card" style="margin-bottom:12px;overflow:hidden;">
        <div style="font-size:11px;font-weight:500;color:var(--ink3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px;">Users</div>
        <div class="overflow"><table><thead><tr><th>Name</th><th>Role</th><th>Premium</th><th></th></tr></thead><tbody>
          ${(users||[]).map(u=>`<tr><td style="color:var(--ink);font-size:12px;">${u.full_name}</td><td><span class="tag ${u.role==='admin'?'tag-red':u.role==='facilitator'?'tag-gold':'tag-blue'}" style="font-size:10px;">${u.role}</span></td><td><button onclick="togglePrem('${u.id}',${u.is_premium})" style="background:${u.is_premium?'#D1FAE5':'var(--bg3)'};color:${u.is_premium?'#065F46':'var(--ink2)'};border:none;border-radius:6px;padding:3px 8px;font-size:11px;cursor:pointer;">${u.is_premium?'✓ Premium':'Free'}</button></td><td><button onclick="delUsr('${u.id}')" style="background:#FEE2E2;color:#991B1B;border:none;border-radius:6px;padding:3px 8px;font-size:11px;cursor:pointer;">Del</button></td></tr>`).join('')}
        </tbody></table></div>
      </div>
      <div class="card" style="margin-bottom:12px;overflow:hidden;">
        <div style="font-size:11px;font-weight:500;color:var(--ink3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px;">Payments</div>
        ${!(payments||[]).length?'<p style="font-size:13px;color:var(--ink3);">No payments yet.</p>':`<div class="overflow"><table><thead><tr><th>Amount</th><th>Ref</th><th>Date</th></tr></thead><tbody>${(payments||[]).map(p=>`<tr><td style="color:#065F46;font-weight:500;">₦${(p.amount/100).toLocaleString()}</td><td style="font-size:10px;font-family:monospace;color:var(--ink3);">${p.reference?.slice(0,16)}…</td><td style="font-size:11px;">${new Date(p.created_at).toLocaleDateString()}</td></tr>`).join('')}</tbody></table></div>`}
      </div>`;
  }else{
...
async function createCohort(){
  const n=document.getElementById('cName').value.trim(),o=document.getElementById('cOrg').value.trim();
  if(!n)return;const b=document.getElementById('cBtn');b.textContent='Creating…';b.disabled=true;
  await sb.from('cohorts').insert({name:n,organisation:o,facilitator_id:S.user.id});renderDash();
}
...
// AI ENGINE
const AI_SYSTEM=`You are the NeuralFusion™ AI Engine — an expert guide for the NeuralFusion™ cognitive skill framework created by Life Edet.

NeuralFusion™ coordinates four thinking modes for professional development and high-pressure decision-making.

THE FOUR THINKING MODES:
1. Analytical — logical, data-driven, systematic reasoning
2. Intuitive — fast, pattern-based, experiential judgment  
3. Associative — lateral, creative, connection-making thinking
4. Reflective — meta-cognitive, self-aware, evaluative thinking

THE CORE LOOP: Scan → Activate → Fuse → Reflect
- Scan: read the cognitive demands of the situation
- Activate: engage the appropriate mode(s)
- Fuse: integrate outputs across modes for a coherent decision
- Reflect: review the outcome and recalibrate

THE FUSE TRIGGER: The moment of cognitive fluency when thinking modes integrate seamlessly and a clear, confident decision emerges. Not forced — it arises from practiced mode coordination.

MODE ACTIVATION SIGNALS:
- Analytical: complex data, competing options, structured evaluation needed
- Intuitive: time pressure, strong emotional signal, familiar pattern recognition
- Associative: stuck on a problem, need novelty, conventional approaches failing
- Reflective: post-decision review, preparing for difficulty, sensing cognitive drift

THE THREE STABILISERS:
1. Grounding — returning to factual anchor points when thinking becomes chaotic
2. Pacing — controlling the speed of cognition to match decision urgency
3. Integration — ensuring all relevant modes have contributed before deciding

EMERGENCY RESET PROTOCOL: Stop → Breathe → Ground (name 3 facts you know for certain) → Select one mode → Proceed. Used in high-stress, high-stakes moments.

CFI — COGNITIVE FRAGMENTATION INDEX: Measures degree of fragmented thinking across 5 dimensions (A–E), 13 items. Pre/post scores measure the Clarity Delta — improvement in cognitive integration after training.

SYNTHESIS FRAMEWORK: Structured workspace for multi-mode problem solving. Input a problem → work through each mode's perspective → arrive at a fused decision.

Your role: Answer questions about NeuralFusion™ clearly and accurately. Use the framework's own terminology. Be concise but thorough. Format with **bold** for key terms. Do not answer questions unrelated to NeuralFusion™ or cognitive frameworks — politely redirect.`;

let aiMsgs=[],aiLoading=false,aiKey='';

function aiInit(){
  const saved=sessionStorage.getItem('nf_ai_key');
  if(saved){aiKey=saved;aiShowChat();}
  else{
...
function aiActivate(){
  ...
  const err=document.getElementById('aiSetupErr');
  err.style.display='none';
  if(!k||!k.startsWith('sk-ant-')){err.style.display='block';err.textContent='Invalid key. Anthropic keys start with sk-ant-';return;}
  aiKey=k;sessionStorage.setItem('nf_ai_key',k);aiShowChat();
}

function aiShowChat(){
  document.getElementById('aiSetup').style.display='none';
  document.getElementById('aiChat').style.display='flex';
  document.getElementById('aiStatusDot').style.background='#22c55e';
  document.getElementById('aiStatusDot').style.boxShadow='0 0 8px #22c55e';
  document.getElementById('aiStatusTxt').textContent='Online';
}

function aiReset(){
  sessionStorage.removeItem('nf_ai_key');aiKey='';aiMsgs=[];
  document.getElementById('aiMessages').innerHTML='';
  document.getElementById('aiKeyInput').value='';
  document.getElementById('aiChat').style.display='none';
  document.getElementById('aiSetup').style.display='flex';
  document.getElementById('aiStatusDot').style.background='#ccc';
  document.getElementById('aiStatusDot').style.boxShadow='none';
  document.getElementById('aiStatusTxt').textContent='AI Engine';
  document.getElementById('aiChips').style.display='flex';
}

function aiUseChip(btn){
...
function aiAppend(role,html){
  ...
  el.className=role==='user'?'ai-msg-user':'ai-msg-ai';
  el.innerHTML=html;
  box.appendChild(el);
...
function aiTyping(){
  const box=document.getElementById('aiMessages');
  const el=document.createElement('div');
  el.id='aiTyping';el.className='ai-typing';
  el.innerHTML='<span></span><span></span><span></span>';
  box.appendChild(el);box.scrollTop=box.scrollHeight;
}

async function aiSend(){
  if(aiLoading||!aiKey)return;
  const inp=document.getElementById('aiInput');
  const text=inp.value.trim();if(!text)return;
  inp.value='';inp.style.height='auto';
  document.getElementById('aiChips').style.display='none';
  document.getElementById('aiSendBtn').style.opacity='.4';
  aiLoading=true;
  aiAppend('user',text);
  ...
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{
      ...
      body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,system:AI_SYSTEM,messages:aiMsgs})
    });
    const data=await res.json();
    const t=document.getElementById('aiTyping');if(t)t.remove();
    if(data.error){aiAppend('ai','⚠️ '+(data.error.message||'API error. Check your key.'));}
    else{const reply=data.content?.[0]?.text||'Something went wrong.';aiMsgs.push({role:'assistant',content:reply});aiAppend('ai',aiMd(reply));}
  }catch(e){
    const t=document.getElementById('aiTyping');if(t)t.remove();
    aiAppend('ai','⚠️ Connection error. Please check your network.');
  }
  aiLoading=false;
